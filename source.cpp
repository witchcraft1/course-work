...
struct node {
	int value;                                      //ключ нового элемента
	int nodeHeight;                              // его высота
...
//функция которая возвращает высоту дерева если оно есть или -1 если его нет
int height(node *Node) {
	return Node ? Node->nodeHeight : -1;
}                                

//высота узла 
void changeHeight(node *Node) {         
	int HeightLeft = height(Node->left); //Node->left->height;
	int HeightRight = height(Node->right); //Node->right->height;

	Node->nodeHeight = (HeightLeft > HeightRight ? HeightLeft : HeightRight) + 1;
}

//разница уровней левого и правого потомка
int difference_height_right_left_child(node *Node) {                         
	return  height(Node->right)-height(Node->left);
}

// БАЛАНСИРОВКА УЗЛОВ. Для этого используются повороты , всего есть 4 : 2 малых:правый и левый, 2 больших: правый и левый
...
node* balance(node *&Node) 
{
	...
	if (difference_height_right_left_child(Node) == 2)
	{
		if (difference_height_right_left_child(Node->right) < 0)
			return left_big_rotation(Node);//Node->right = right_small_rotation(Node->right);
		return left_small_rotation(Node);
	}
	if (difference_height_right_left_child(Node) == -2)
	{
		if (difference_height_right_left_child(Node->left) > 0)
			return right_big_rotation(Node);//Node->left = left_small_rotation(Node->left);
		return right_small_rotation(Node);
	}
	return Node; // балансировка не нужна
}

 // вставка ключа elem в дерево с корнем Tree
//основная функция для добавления элементов в дерево сразу с балансировкой дерева
...
// вспомагательная функция которая используется в функции копирования дерева
...
//удаления улюча elem с дерева Тrее
		...
		if (Tree == nullptr) {
			...
			else
			{
				if (Tree->left == nullptr && Tree->right == nullptr) {
					...
					if (tmp == ttmp) Tree->right = tmp->right;
					else ttmp->left = tmp->right;
					...
					if (tmp == ttmp) Tree->left = tmp->left;
					else ttmp->right = tmp->left;
					Tree->value= tmp->value;
					delete tmp;
					//
					return Tree;
...
void copy(node *&tree, node*&tree2) {
	if (tree == nullptr)  return; //cout << "Tree doesn\'t have any element" << endl;

	addNode(tree->value, tree2);
	if (tree->left)	copy(tree->left, tree2);
	if (tree->right) copy(tree->right, tree2);
}
...
void test_copy(node *&Tree, node *&Tree2) {
	if (Tree == nullptr && Tree2 == nullptr)  return; //cout << "Tree doesn\'t have any element" << endl;
	assert(Tree && Tree2);
	assert(Tree->value == Tree2->value);

	//addNode(tree->data, &tree2);
	if (Tree->left || Tree2->left)	test_copy(Tree->left, Tree2->left);
	if (Tree->right || Tree2->right)    test_copy(Tree->right, Tree2->right);
}

// функция вывода на экран
void show(node *tree, int spaces) {
	if (!tree) cout << "Tree doesn\'t exist" << endl;
	while(tree){
	if (tree->right) {
...
int main() { 
  ...
  // deleteNode(150, Tree2);  deleteNode(50000, Tree2);  deleteNode(5000, Tree2);
   show(Tree2, 0);
